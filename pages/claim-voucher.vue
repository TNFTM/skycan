...
export default {
	...
	mounted() {
		...
		this.whitelisted = require('assets/whitelisted.json');
	},
	methods: {
		async claim() {
			this.loading = true;
			if (this.whitelisted.includes(this.$wallet.publicKey)) {
				window.localStorage.setItem(this.$wallet.publicKey, 'true');
				...
				if(true) {
					...
					if (holdingAmount >= this.participants[this.$wallet.publicKey]) {
						...
						alert("Did you do yield farming of PSOL on Cropper Finance, transfert to an other wallet or any other problems ? claim your voucher manually.\n" +
							"\n" +
							"Complete this form: https://forms.gle/QrPwX9gsQD3jUgsX6" +
							"\n" +
							"You will be redirect to the form automatically");
							window.open("https://forms.gle/QrPwX9gsQD3jUgsX6", "_blank");
					}
				}
			}
		},
		...
		haveVoucher() {
			return window.localStorage.getItem(this.$wallet.publicKey) !== null;
		}
	}
}
</script>
