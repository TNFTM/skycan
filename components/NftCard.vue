<template>
	<article
		@mouseenter="play"
		@mouseleave="pause"
		:class="`${limitedCount > 0 && remaining === 0 ? '' : 'hoverable'} relative mx-4 lg:mx-0 flex-col rounded-xl shadow-lg scale-125 bg-white bg-opacity-5 text-white overflow-hidden`">
		<div v-if="limitedCount > 0 && remaining === 0" class="absolute flex pt-24 items-start justify-center w-full h-full z-50 backdrop-filter backdrop-blur">
			<img class="w-48" src="https://cdn-icons-png.flaticon.com/512/2331/2331975.png" alt="sold-out" />
		</div>
		<div class="p-5">
			<div class="relative">
				<!--<span v-if="limitedCount === 0" class="absolute top-0 right-0 m-3 items-center justify-center px-2 py-1 font-medium leading-none indigo-500 bg-purple-500 bg-opacity-50 rounded">
					{{ remaining }}/∞
				</span>
				<span v-else class="absolute top-0 right-0 m-3 items-center justify-center px-2 py-1 font-medium leading-none indigo-500 bg-purple-500 bg-opacity-50 rounded">
		...
		<div class="flex-1 flex flex-col justify-between px-8 pb-8 gap-y-4">
			...
			<div v-if="limitedCount === 0 || remaining > 0" class="rounded-md shadow">
...
export default {
	...
	async mounted() {
		this.remaining = this.limitedCount;
		let count = await this.$axios.$get(`https://parasol-finance.azurewebsites.net/counter/${this.name}`);
		if(this.limitedCount > 0) {
			this.remaining = this.limitedCount - count;
		}
		else {
