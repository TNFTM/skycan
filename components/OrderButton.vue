<template>
 	<button v-if="this.$wallet.isConnected" data-click="order" class="flex items-center w-full uppercase justify-center mt-5 px-5 py-3 text-base font-medium rounded-md bg-gradient-primary text-white">
		<span v-if="should() > 0">Order is Temporarily Disabled</span>
		<!--<span v-if="should() > 0">Buy NFT Now For {{ amount.toLocaleString('en-US', { minimumFractionDigits: 0 }) }} PSOL</span>-->
		<SmallCountdown v-else />
	</button>
...
export default {
	...
	methods: {
		async order() {
			if (this.should() > 0) {
				await this.orderTiers(this.name, this.amount);
			}
		},
		should() {
			 return new Date() - new Date("Tue, 1 Feb 2022 21:00:21 GMT")
		},
